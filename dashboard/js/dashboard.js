...
function resumenInformacionTaller(){
    ...
    http.onreadystatechange = function(){
        if(this.readyState == 4 && this.status ==200){
              console.log(this.responseText);
           document.getElementById("divrResumenInformacionTaller").innerHTML  = this.responseText;
        //    document.getElementById("modalBodyTractores").innerHTML  = 'llego al js';
        }
    };
    http.open("POST",url);
    http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    http.send("opcion=resumenInformacionTaller"
        // +'&idCliente='+idCliente
        //         +'&idTractor='+idTractor
        //         +'&observaciones='+observaciones
    );
